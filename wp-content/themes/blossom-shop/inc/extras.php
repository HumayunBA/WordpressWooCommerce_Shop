...
function blossom_shop_tag(){
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $tags_list ) {
			...
			printf( '<div class="cat-tags" itemprop="about">' . esc_html__( '%1$TAGS:%2$s %3$s', 'blossom-shop' ) . '</div>', '<span class="tag-title">', '</span>', $tags_list );
		}
	}
}
endif;
...
function blossom_shop_singular_post_thumbnail() {
    ...
    if( is_singular() ){
        ...
        if( is_single() ){
            ...
            $background_image = get_template_directory_uri() .'/images/page-header-bg.jpg';
            if( has_post_thumbnail() ) :
